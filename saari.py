...
#    mcBIa = np.zeros(N)
    ...
    # Monte Carlo loop
#    for i in range(N):
#        BIc,BIm,BIs,BIa,BIIc,BIIm,BIIs,BIIa = band_params(wavx,refly+mcerr[i,:],xnew,1./err)
...
print(np.mean(mcBIc),np.std(mcBIc))
print(np.mean(mcBIm),np.std(mcBIm))
print(np.mean(mcBIs),np.std(mcBIs))
print(np.mean(mcBIIc),np.std(mcBIIc))
print(np.mean(mcBIIm),np.std(mcBIIm))
print(np.mean(mcBIIs),np.std(mcBIIs))
print(np.mean(mcBAR),np.std(mcBAR))
